...
# All credit for this script goes to Spaceinvader One (https://github.com/SpaceinvaderOne), I have simply adapted this script to work
# with Hetzner Storagebox as they have some subtle differences over standard SSH.
...
DEST_SERVER_IP="uxxxxxx@uxxxxxx.your-storagebox.de"  # The IP address or hostname of the remote server.
SSH_PORT=23                     # The SSH port of the remote server. 22 is standard. Hetzner use port 23.
...
check_ssh_connection() {
  if [[ "${DEST_TYPE,,}" == "remote" || "${DEST_TYPE,,}" == "both" ]]; then
    ...
    # The below doesn't work with Hetzner, I need to work on an alternative that will exit on error.
